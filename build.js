...
function build(browser, buildMode) {
    return new Promise(function(resolve, reject) {
        ...
        fs.mkdirsSync(destDirectory + '/contentscript');
        fs.mkdirsSync(destDirectory + '/contentscript/css');
        fs.mkdirsSync(destDirectory + '/contentscript/vendor');

        fs.copySync(binDirectory + '/assets', destDirectory + '/assets');
        fs.copySync(binDirectory + '/data', destDirectory + '/data');
        fs.copySync(binDirectory + '/metadata', destDirectory);
        
        fs.copySync(binDirectory + '/contentscript/vendor', destDirectory + '/contentscript/vendor');
        ...
        buildPromises.push(buildHTML(binDirectory + '/options/options.pug', destDirectory + '/options/options.html'));
        ...
        Promise.all(buildPromises).then(function() {
            if (buildMode === 'test') {
                ...
                zipFolder(destDirectory, RELEASE_DIRECTORY + '/TCR_' + browser + '_' + projectJSON.version + '.zip').then(function () {
                    resolve();
                });
            }
        });
